...
describe Hydra::Derivatives::ActiveEncodeDerivatives do
  describe '.create' do
    ...
    it 'calls the processor with the right arguments' do
      expect(Hydra::Derivatives::Processors::ActiveEncode).to receive(:new).with(file_path, low_res_video, { output_file_service: Hydra::Derivatives::Fedora::PersistExternalFileOutputFileService }).and_return(processor)
    ...
    context 'with a custom encode class' do
      ...
      it 'calls the processor with the right arguments' do
        expect(Hydra::Derivatives::Processors::ActiveEncode).to receive(:new).with(file_path, low_res_video, { output_file_service: Hydra::Derivatives::Fedora::PersistExternalFileOutputFileService, encode_class: TestEncode }).and_return(processor)
        expect(processor).to receive(:process)
        described_class.create(video_record, options)
      end
    end
